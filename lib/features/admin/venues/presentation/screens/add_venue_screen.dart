...
class _AddVenueScreenState extends State<AddVenueScreen> {
  ...
  List<GroundModel> _detectGrounds(String name) {
    ...
    if (lower.contains('padel'))
      return [GroundModel(groundName: 'Padel Court', groundType: 'Padel', courtCount: 4)];
    if (lower.contains('cricket') || lower.contains('stadium'))
      return [GroundModel(groundName: 'Main Cricket Ground', groundType: 'Cricket', courtCount: 1)];
    if (lower.contains('football') || lower.contains('turf'))
      return [GroundModel(groundName: 'Football Turf', groundType: 'Football', courtCount: 1)];
    if (lower.contains('tennis'))
      return [GroundModel(groundName: 'Tennis Court', groundType: 'Tennis', courtCount: 2)];
    if (lower.contains('badminton'))
      return [GroundModel(groundName: 'Badminton Court', groundType: 'Badminton', courtCount: 6)];
    return [GroundModel(groundName: 'Main Ground', groundType: 'General', courtCount: 1)];
  }
  ...
  Future<void> _save() async {
    ...
    // Location validation — must have either lat/lng OR a mapUrl
    if (_latCtrl.text.trim().isEmpty && _mapUrlCtrl.text.trim().isEmpty) {
      _showError('Please select a location from search or enter a Map URL manually');
      return;
    }
    if (_grounds.isEmpty) {
    ...
    await Future.delayed(const Duration(milliseconds: 600)); // TODO: real API
                  ...
                  children: [
                    ...
                    Row(children: [
                      Expanded(child: Column(
                        ...
                        children: [
                          _FormLabel("Latitude"),
                          _FormField(controller: _latCtrl, hint: "e.g. 18.9388",
                      ...
                      Expanded(child: Column(
                        ...
                        children: [
                          ...
                          _FormField(controller: _lngCtrl, hint: "e.g. 72.8259",
                              icon: Icons.my_location,
                              readOnly: _locationVerified),
                        ],
                      )),
