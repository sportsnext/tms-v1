...
class _EditSportScreenState extends State<EditSportScreen> {
  ...
  void initState() {
    ...
    _icon = _icons.contains(s.icon) ? s.icon : _icons.first;
    _preset = 'Custom';
    // Scoring
  ...
  Future<void> _save() async {
    ...
    try {
      final url = Uri.parse(
        "https://dev.sports-next.com/api/sports/${widget.sport.id}"
      );
  ...
  Widget build(BuildContext context) {
    return _SportFormDialog(
      title: "Edit Sport",
      subtitle:
          '"${widget.sport.name}" · v${widget.sport.version} → v${widget.sport.version + 1}',
      isEdit: true,
      saving: _saving,
      ...
      child: Form(
        ...
        child: _SportFormBody(
          ...
          onCategory: (v) => _categoryCtrl.text = v,
