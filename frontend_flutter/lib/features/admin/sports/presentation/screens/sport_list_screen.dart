...
class _SportsListScreenState extends State<SportsListScreen> {
  ...
  List<SportModel> get _filtered => _sports
      .where(
        (s) =>
  ...
  Future<void> fetchSports({int page = 1}) async {
    try {
      final response = await http.get(
        Uri.parse("$baseUrl/sports?page=$page&search=$_searchQuery"),
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        if (decoded is List) {
          ...
          venueData = decoded['data'] ?? [];
        }

        setState(() {
          _sports = venueData
              .map((event) => SportModel.fromJson(event))
              .toList();
                ...
                children: [
                  ...
                  Expanded(
                    child: ElevatedButton(
                      onPressed: () async {
                        await http.delete(
                          Uri.parse("$baseUrl/sports/${sport.id}"),
                          headers: {
                            "Authorization": "Bearer $token",
                            "Content-Type": "application/json",
                          },
                        ...
                        _showSnack('"${sport.name}" deleted successfully');
                      },
  ...
  void _openEdit(SportModel sport) {
    showDialog(
      ...
      builder: (_) => EditSportScreen(
        sport: sport,
        onSave: (SportModel updated) async {
          fetchSports();
            ...
            children: [
              Column(
                ...
                children: [
                  ...
                  Text(
                    "Define sports and configure scoring rules",
          ...
          Row(
            children: [
              ...
              _StatCard(
                label: "With Tiebreak",
                value:
                    "${_sports.where((s) => s.scoringRules.hasTieBreak).length}",
    ...
    child: Center(
      child: Column(
        children: [
          ...
          const Text(
            "Click '+ Add Sport' to get started",
