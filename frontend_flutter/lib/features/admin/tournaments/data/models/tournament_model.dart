...
class FixtureModel {
  ...
  Color get statusColor {
    switch (status) {
      ...
      case 'completed':
        return const Color(0xFF111827); // ⚫ DARK (CLEARLY DIFFERENT)
      case 'cancelled':
  ...
  factory FixtureModel.fromJson(Map<String, dynamic> json) {
    ...
    return FixtureModel(
      id: json['id'].toString(),
      eventGroupId: json['event_group_id'].toString(),
      round: json['round'] ?? '',
      matchNumber: json['match_number'] ?? 0,
      roundIndex: json['round_index'] ?? 0,
      matchIndex: json['match_index'] ?? 0,
      ...
      venueName: '',
      court:
          (json['court'] ?? json['court_name'] ?? json['venue_name'])
              ?.toString() ??
          '',

      status: status,
      isLive: status.trim().toLowerCase() == 'live',

      sets: (json['sets'] is List)
          ? (json['sets'] as List).map((s) => SetScore.fromJson(s)).toList()
          : [],
      setsWonA: json['sets_won_a'] ?? 0,
      setsWonB: json['sets_won_b'] ?? 0,

      winnerId: json['winner_team_id']?.toString() ?? '',
...
class ParticipantModel {
  ...
  String get displayPlayers =>
      playerNames.isEmpty ? name : playerNames.join(' /');
  String get initials {
  ...
  factory ParticipantModel.fromJson(Map<String, dynamic> json) {
    ...
    final playersRaw = (team != null && team['players'] is List)
    ...
    return ParticipantModel(
      ...
      // 🔥 FIX (IMPORTANT)
      name: json['name'] ?? team?['name'] ?? '',

      playerNames: playersRaw
          .where((p) => p != null && p['full_name'] != null)
          .map((p) => p['full_name'].toString())
          .toList(),
...
class StandingRow {
  ...
  const StandingRow({
    ...
    required this.won,
    required this.lost,
    required this.gameDiff,
    required this.points,
  });

  // 🔥 ADD THIS (VERY IMPORTANT)
  factory StandingRow.fromJson(Map<String, dynamic> json, String name) {
    return StandingRow(
      participantId: json['team_id'].toString(),
      participantName: name, // 🔥 from participants
...
class EventGroup {
  ...
  final List<FixtureModel> manualFixtures;
      ...
      for (int j = i + 1; j < parts.length; j++) {
        fixtures.add(
          FixtureModel(
            ...
            matchNumber: matchNo,
            roundIndex: 0,
            matchIndex: matchNo - 1,
            teamAId: parts[i].id,
            teamAName: parts[i].name,
            teamBId: parts[j].id,
            teamBName: parts[j].name,
            date: '',
            time: '',
            court: '',
            status: 'scheduled',
  ...
  static List<FixtureModel> generateKnockout(
    ...
    for (int i = 0; i < size; i += 2) {
      ...
      fixtures.add(
        FixtureModel(
          id: 'f_${groupId}_r0_${i ~/ 2}',
          eventGroupId: groupId,
          round: _roundLabel(size),
          matchNumber: matchNo++,
          roundIndex: 0,
          matchIndex: i ~/ 2,
          teamAId: a.id,
    ...
    while (roundSize >= 2) {
      ...
      for (int i = 0; i < count; i++) {
        fixtures.add(
          FixtureModel(
            id: 'f_${groupId}_r${roundIdx}_$i',
            eventGroupId: groupId,
            round: _roundLabel(roundSize),
            matchNumber: matchNo++,
            roundIndex: roundIdx,
            matchIndex: i,
            teamAId: '',
  ...
  List<StandingRow> get standings {
    ...
    for (final f in fixtures) {
      ...
      final a = map[f.teamAId];
      final b = map[f.teamBId];
  ...
  EventGroup copyWith({
    ...
    List<FixtureModel>? manualFixtures,
    int? stageId,
  }) => EventGroup(
    id: id ?? this.id,
    ...
    manualFixtures: manualFixtures ?? List.from(this.manualFixtures),
  ...
  static const List<String> genders = ['Men', 'Women', 'Mixed'];

  factory EventGroup.fromJson(Map<String, dynamic> json) {
    // 🔥 SAFE MATCHES EXTRACTION (CORE FIX)
    final rawFixtures = json['fixtures'] ?? json['matches'];

    List<FixtureModel> parsedFixtures = [];

    if (rawFixtures is List) {
      parsedFixtures = rawFixtures.map((e) {
        return FixtureModel.fromJson(Map<String, dynamic>.from(e));
      }).toList();
    }
    return EventGroup(
      id: json['id'].toString(),
      stageId: json['stage_id'] != null
          ...
          : (json['stage'] != null
          ? int.tryParse(json['stage']['id'].toString())
          : null),
      eventName: json['event_name'] ?? '',
      sportName: json['sport_name'] ?? '',
      format: json['format'] ?? 'round_robin',
      participantType: json['participant_type'] ?? 'Team',
      gender: (json['gender'] == 'Open' || json['gender'] == null)
          ? 'Men'
          : json['gender'],
      ...
      // 🔥 ONLY CLEAN FIXTURES (NO MIXED DATA POSSIBLE)
...
class TournamentModel {
  ...
  String get effectiveVenue {
    if (venueManual.isNotEmpty) return venueManual;
  ...
  int get completedMatches => eventGroups.fold(
    0,
    (s, g) => s + g.fixtures.where((f) => f.isCompleted).length,
  );

  int get totalParticipants =>
      eventGroups.fold(0, (s, g) => s + g.participants.length);

  int get liveMatches => eventGroups.fold(
    0,
    (s, g) =>
        s +
        g.fixtures.where((f) => f.status.trim().toLowerCase() == 'live').length,
  );

  bool get hasLive => liveMatches > 0;
  List<FixtureModel> get allFixtures => [
    for (final g in eventGroups) ...g.fixtures,
  ];
  ...
  TournamentModel copyWith({
    ...
    id: id ?? this.id,
    name: name ?? this.name,
    subTypeLabel: subTypeLabel ?? this.subTypeLabel,
    ...
    fixtureImage: fixtureImage ?? this.fixtureImage,
  ...
  factory TournamentModel.fromJson(Map<String, dynamic> json) {
    print(json['event_groups']);
    final rawBanner = json['banner'];

    String fixedBanner = '';

    if (rawBanner != null && rawBanner.toString().isNotEmpty) {
      final b = rawBanner.toString();

      if (b.startsWith('http')) {
        fixedBanner = b;
      } else {
        fixedBanner =
            "https://dev.sports-next.com/storage/${b.replaceAll("\\", "/")}";
      }
    }

    final rawFixture = json['fixture_image'];

    String? fixedFixtureImage;

    if (rawFixture != null && rawFixture.toString().isNotEmpty) {
      final f = rawFixture.toString();

      if (f.startsWith('http')) {
        fixedFixtureImage = f;
      } else {
        fixedFixtureImage =
            "https://dev.sports-next.com/storage/${f.replaceAll("\\", "/")}";
      }
    }
    ...
    return TournamentModel(
      ...
      venueManual: json['venue_manual'] ?? '',

      startDate: json['start_date'] ?? '',
      endDate: json['end_date'] ?? '',
      registrationDueDate: json['registration_due_date'] ?? '',
      ...
      banner: fixedBanner,
      fixtureImage: fixedFixtureImage,
      description: json['description'] ?? '',
      status: json['status'] ?? 'draft',

      eventGroups:
          ((json['event_groups'] ?? json['eventGroups']) as List? ?? [])
              .map((e) => EventGroup.fromJson(e))
              .toList(),
...
class TournamentSeeds {
  ...
  static final _womenParts = [
    ParticipantModel(
      id: 'wp1',
      name: 'Nimisha / Sneha',
      playerNames: ['Nimisha Idekar', 'Sneha Bhavsar'],
      seed: '1',
    ),
    ParticipantModel(
      id: 'wp2',
      name: 'Priya / Kavya',
      playerNames: ['Priya Sharma', 'Kavya Nair'],
      seed: '2',
    ),
    ParticipantModel(
      id: 'wp3',
      name: 'Riya / Neha',
      playerNames: ['Riya Kapoor', 'Neha Joshi'],
      seed: '3',
    ),
    ParticipantModel(
      id: 'wp4',
      name: 'Divya / Ananya',
      playerNames: ['Divya Mehta', 'Ananya Rao'],
      seed: '4',
    ),
  ];
  static final _womenFixtures = () {
    ...
    list[0] = list[0].copyWith(
      ...
      time: '10:00',
      court: 'Court 1',
      venueName: 'Prime Padel Club',
    );
    list[1] = list[1].copyWith(
      status: 'completed',
      sets: [
        SetScore(scoreA: 4, scoreB: 6),
        SetScore(scoreA: 7, scoreB: 5),
        SetScore(scoreA: 6, scoreB: 4),
      ],
      setsWonA: 2,
      setsWonB: 1,
      winnerId: 'wp1',
    ...
    list[3] = list[3].copyWith(
      date: '2026-03-14',
      time: '14:30',
      court: 'Court 2',
      venueName: 'Prime Padel Club',
    );
    list[4] = list[4].copyWith(
      date: '2026-03-15',
      time: '10:00',
      court: 'Court 1',
      venueName: 'Prime Padel Club',
    );
    list[5] = list[5].copyWith(
      date: '2026-03-15',
      time: '11:30',
      court: 'Court 3',
      venueName: 'Prime Padel Club',
  ...
  static final _menFixtures = () {
    ...
    if (qf2 != -1)
      list[qf2] = list[qf2].copyWith(
        status: 'live',
        isLive: true,
        sets: [SetScore(scoreA: 4, scoreB: 6)],
        setsWonA: 0,
        setsWonB: 1,
        date: '2026-03-14',
        time: '11:00',
  ...
  static final _mixedParts = [
    ParticipantModel(
      id: 'mx1',
      name: 'Arjun / Priya',
      playerNames: ['Arjun Mehta', 'Priya Sharma'],
      seed: '1',
    ),
    ParticipantModel(
      id: 'mx2',
      name: 'Rahul / Divya',
      playerNames: ['Rahul Gupta', 'Divya Mehta'],
      seed: '2',
    ),
    ParticipantModel(
      id: 'mx3',
      name: 'Vikram / Riya',
      playerNames: ['Vikram Singh', 'Riya Kapoor'],
      seed: '3',
    ),
    ParticipantModel(
      id: 'mx4',
      name: 'Aditya / Ananya',
      playerNames: ['Aditya Joshi', 'Ananya Rao'],
      seed: '4',
    ),
  ];
  ...
  static final List<TournamentModel> all = [
    TournamentModel(
      id: 'trn1',
      name: 'Mumbai Padel Open 2026',
      subTypeLabel: 'Open Championship',
      sportId: 'sp1',
      sportName: 'Padel',
      eventId: 'e1',
      eventName: 'Padel Open',
      venueId: 'v2',
      venueName: 'Prime Padel Club',
      startDate: '2026-03-14',
      endDate: '2026-03-16',
      registrationDueDate: '2026-03-10',
      contactName: 'Padel Park Mumbai',
      contactEmail: 'info@padelpark.in',
      ...
      banner: '',
      description:
          'Annual Mumbai Padel Open — featuring Women\'s Doubles, Men\'s Singles Knockout and Mixed Doubles. India\'s premier padel championship.',
      status: 'published',
      eventGroups: [
        EventGroup(
          id: 'eg1',
          eventName: "Women's Doubles",
          sportName: "Women's Doubles",
          format: 'round_robin',
          participantType: 'Pair',
          gender: 'Women',
          participants: _womenParts,
          fixtures: _womenFixtures,
        ),
        EventGroup(
          id: 'eg2',
          eventName: "Men's Singles",
          sportName: "Men's Singles",
          format: 'knockout',
          participantType: 'Individual',
          gender: 'Men',
          participants: _menParts,
          fixtures: _menFixtures,
        ),
      ],
      sponsors: const [],
  ...
  static const List<Map<String, String>> sports = [
    {'id': 'sp1', 'name': 'Padel'},
    {'id': 'sp2', 'name': 'Tennis'},
    {'id': 'sp3', 'name': 'Badminton'},
    {'id': 'sp4', 'name': 'Squash'},
    {'id': 'sp5', 'name': 'Cricket'},
    {'id': 'sp6', 'name': 'Table Tennis'},
  ];
  static const List<Map<String, String>> events = [
    {'id': 'e1', 'name': 'Padel Open'},
    {'id': 'e2', 'name': 'Cricket Cup'},
    {'id': 'e3', 'name': 'Tennis Open'},
    {'id': 'e4', 'name': 'Badminton Open'},
    {'id': 'e5', 'name': 'Club Championship'},
  ];
  static const List<Map<String, String>> venues = [
    {'id': 'v1', 'name': 'Wankhede Stadium'},
    {'id': 'v2', 'name': 'Prime Padel Club'},
    {'id': 'v3', 'name': 'DY Patil Stadium'},
    {'id': 'v4', 'name': 'Brabourne Stadium'},
