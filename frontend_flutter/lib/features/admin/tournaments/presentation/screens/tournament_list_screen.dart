...
class _TournamentListScreenState extends State<TournamentListScreen> {
  ...
  Future<void> loadTournaments() async {
    try {
      final data = await TournamentService.getTournaments();
  ...
  void _openCreate() async {
    ...
    if (result == true) {
      await loadTournaments(); // 🔥 FORCE RELOAD
    }
  }

  void _openEdit(TournamentModel t) async {
    final result = await showDialog(
      context: context,
      barrierDismissible: false,
      builder: (_) => CreateTournamentScreen(
    ...
    if (result == true) {
      await loadTournaments(); // 🔥 THIS IS THE FIX
    }
  }

  void _openView(TournamentModel t) async {
    try {
      print("CLICKED TOURNAMENT ID: ${t.id}");
      ...
      final data = res['data'];
      final updated = TournamentModel.fromJson(data);

      print("EVENT GROUPS COUNT: ${updated.eventGroups.length}");
      ...
      print("VIEW ERROR: $e");
      _snack("Failed to load tournament");
    }
  }
                    ...
                    child: ElevatedButton(
                      onPressed: () async {
                        try {
                          ...
                          await loadTournaments(); // 🔥 REFRESH FROM DB
  ...
  void _togglePublish(TournamentModel t) async {
    final newStatus = t.status == 'published' ? 'draft' : 'published';

    try {
      final res = await TournamentService.updateStatus(t.id, newStatus);

      final updated = t.copyWith(status: res['status'] ?? newStatus);
  ...
  void _completeTournament(TournamentModel t) async {
        ...
        content: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                'Mark "${t.name}" as completed? This will move it to Past Tournaments.',
                textAlign: TextAlign.center,
                style: TextStyle(
                  fontSize: 13,
                  color: Colors.grey.shade600,
    ...
    try {
      ...
      _snack('"${t.name}" marked as completed');
    } catch (e) {
      _snack('Failed to complete tournament');
    }
  }
  ...
  void initState() {
    super.initState();
    print("🟡 INIT TOKEN: ${AppSession.token}");
    print("ROLE ID: ${AppSession.roleId}");
    loadTournaments();
  }
            ...
            children: [
              Column(
                ...
                children: [
                  ...
                  Text(
                    'Create, manage and track all tournaments',
              ...
              : GridView.builder(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return _TournamentCard(
                      ...
                      onEdit: AppSession.roleId == 5
                          ? null
                          : () => _openEdit(t),
                      onDelete: AppSession.roleId == 5
                          ? null
                          : () => _confirmDelete(t),
                      onTogglePublish: AppSession.roleId == 5
                          ? null
                          : () => _togglePublish(t),
                      onComplete: AppSession.roleId == 5
                          ? null
        ...
        child: Column(
          ...
          children: [
            ...
            ClipRRect(
              ...
              child: (t.banner != null && t.banner!.isNotEmpty)
                  ? Builder(
                      builder: (context) {
                        ...
                        return SizedBox(
                          ...
                          child: HtmlElementView(viewType: 'image-${t.id}'),
                        );
                      },
                    )
                  : Container(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      const Spacer(),
                      Text(
                        '${t.eventGroups.length} event${t.eventGroups.length != 1 ? 's' : ''}',
                        style: TextStyle(
                          fontSize: 11,
                  ...
                  Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          (t.venueName != null && t.venueName!.isNotEmpty)
                              ? t.venueName!
                              : (t.city.isNotEmpty ? t.city : 'No venue'),
                          overflow: TextOverflow.ellipsis,
                          style: TextStyle(
                            fontSize: 11,
                            color: Colors.grey.shade500,
                  ...
                  Row(
                    children: [
                      _MiniStat(
                        ...
                        '${t.eventGroups.fold(0, (s, g) => s + g.fixtures.length)}',
                        'Matches',
                      ),
                      const SizedBox(width: 12),
                      _MiniStat(
                        Icons.people_alt_outlined,
                        '${t.eventGroups.fold(0, (s, g) => s + g.participants.length)}',
                        'Players',
                      ),
                      const SizedBox(width: 12),
                      _MiniStat(
                    ...
                    children: [
                      ...
                      Expanded(
                        child: _CardBtn(
                          ...
                          onTap: () {
                            // 🔥 SET TOURNAMENT ID
                            AppSession.setTournament(t.id.toString());

                            print("✅ SELECTED TOURNAMENT: ${t.id}");
                      ...
                      // ❌ HIDE ALL ACTIONS FOR USER
                      if (AppSession.roleId != 5) ...[
                        const SizedBox(width: 6),

                        // Edit
                        t.status == 'published' || t.status == 'completed'
                            ? _IconAction(
                                Icons.lock,
                                Colors.grey,
                                'Locked',
                        ...
                        // Publish / Unpublish (hide if completed)
                        ...
                        if (t.status != 'completed') const SizedBox(width: 6),

                        // Complete (only for published)
                        ...
                        if (t.status == 'published') const SizedBox(width: 6),
