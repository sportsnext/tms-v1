...
// Place at: lib/features/admin/venues/presentation/screens/add_venue_screen.dart
// pubspec.yaml: add   http: ^1.2.1

class AddVenueScreen extends StatefulWidget {
  final void Function(VenueModel venue) onSave;
  const AddVenueScreen({super.key, required this.onSave});
...
class _AddVenueScreenState extends State<AddVenueScreen> {
  ...
  bool _saving = false;
  bool _locationVerified = false; // true when picked from search
  ...
  Future<void> _searchVenue(String query) async {
    ...
    try {
      ...
      final res = await http.get(
        uri,
        headers: {'User-Agent': 'TMS-Flutter-App/1.0', 'Accept-Language': 'en'},
      );
  ...
  // ── Auto-detect ground type from venue name ───────────────
  List<GroundModel> _detectGrounds(String name) {
    final lower = name.toLowerCase();
    if (lower.contains('padel'))
      return [
        GroundModel(
          groundName: 'Padel Court',
          groundType: 'Padel',
          courtCount: 4,
        ),
      ];
    if (lower.contains('cricket') || lower.contains('stadium'))
      return [
        GroundModel(
          groundName: 'Main Cricket Ground',
          groundType: 'Cricket',
          courtCount: 1,
        ),
      ];
  ...
  void _updateGround(int i, GroundModel updated) {
    setState(() => _grounds[i] = updated);
  }
  ...
  Future<void> _save() async {
    ...
    if (_latCtrl.text.trim().isEmpty && _mapUrlCtrl.text.trim().isEmpty) {
      _showError(
        'Please select a location from search or enter a Map URL manually',
      );
      return;
    }
    ...
    try {
      final url = Uri.parse("https://dev.sports-next.com/api/venues");

      final body = {
        "venue_name": _nameCtrl.text.trim(),
        "location": _addressCtrl.text.trim(),
        "description": _notesCtrl.text.trim(),
        "total_courts": _grounds.fold(0, (sum, g) => sum + g.courtCount),
        "is_active": _status == "Active" ? 1 : 0,
        "grounds": _grounds.map((g) => g.toJson()).toList(),
      };
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        ...
        final newVenue = VenueModel.fromJson(data);
        ...
        _showError("Failed to save venue");
      }
    } catch (e) {
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            _ModalHeader(
              title: "Add Venue",
              subtitle: "Fill in venue details and location",
                ...
                child: Form(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // ── Map URL (shown when NOT verified from search) ──
                      ...
                      Text(
                        "Add the courts or grounds available at this venue",
                        style: TextStyle(
                          fontSize: 12,
              ...
              Expanded(
                ...
                child: TextField(
                  ...
                  onChanged: (v) => onUpdate(
                    ground.copyWith(courtCount: int.tryParse(v) ?? 1),
                  ),
                  controller:
                      TextEditingController(text: "${ground.courtCount}")
                        ..selection = TextSelection.collapsed(
                          offset: "${ground.courtCount}".length,
                        ),
                  style: const TextStyle(fontSize: 13),
                  decoration: _inputDeco("Courts"),
                ),
              ),
            ],
          ),
...
// ── Venue search field — Google Maps style ────────────────────
                  ...
                  Flexible(
                    child: ListView.builder(
                      ...
                      itemBuilder: (_, i) {
                        ...
                        final (primary, secondary) = _splitName(displayName);
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          color: _hovered ? const Color(0xFFF0F5FF) : Colors.transparent,
...
class _FormField extends StatelessWidget {
  ...
  Widget build(BuildContext context) => TextFormField(
    ...
    style: TextStyle(
      fontSize: 14,
      color: readOnly ? Colors.grey.shade500 : const Color(0xFF0A1D4A),
    ),
    decoration: InputDecoration(
      hintText: hint,
      ...
      enabledBorder: OutlineInputBorder(
        ...
        borderSide: BorderSide(
          color: readOnly ? Colors.grey.shade200 : const Color(0xFF93C5FD),
          width: 1.2,
        ),
      ),
      focusedBorder: OutlineInputBorder(
