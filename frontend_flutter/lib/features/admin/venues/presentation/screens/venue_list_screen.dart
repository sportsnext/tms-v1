  ...
  Future<void> fetchVenues({int page = 1}) async {
    try {
      final response = await http.get(
        Uri.parse("$baseUrl/venues?page=?page=$page&search=$_searchQuery"),
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _venues = venueData
              .map((event) => VenueModel.fromJson(event))
              .toList();
  ...
  void _openAdd() {
    showDialog(
      ...
      builder: (_) => AddVenueScreen(
        onSave: (VenueModel newVenue) async {
          await fetchVenues();
  ...
  void _openEdit(VenueModel venue) {
    showDialog(
      ...
      builder: (_) => EditVenueScreen(
        venue: venue,
        onSave: (VenueModel updated) async {
          await fetchVenues();
            ...
            children: [
              Column(
                ...
                children: [
                  ...
                  Text(
                    "Manage match locations and courts",
