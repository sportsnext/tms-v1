...
class _PlayerListScreenState extends State<PlayerListScreen>
  ...
  List<List<PlayerModel>> _duplicateGroups = [];
  ...
  List<PlayerModel> get _filtered => _players.where((p) {
    // ✅ FIXED FILTER LOGIC
    if (_showInactive) {
      if (p.isActive) return false; // show ONLY inactive
    } else {
      if (!p.isActive) return false; // show ONLY active
    }
  ...
  void _toggleActive(PlayerModel p) {
    ...
    _snack(
      p.isActive
          ? '"${p.fullName}" deactivated'
          : '"${p.fullName}" reactivated',
    );
  }

  Future<void> fetchPlayers({int page = 1}) async {
    try {
      final response = await http.get(
        Uri.parse("$baseUrl/players?page=$page&search=$_searchQuery"),
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _players = playerData
              .map((event) => PlayerModel.fromJson(event))
              .toList();

          _currentPage = decoded['current_page'] ?? 1;
          _lastPage = decoded['last_page'] ?? 1;

          _totalPlayers = decoded['total'] ?? _players.length;
          
          _activePlayers = decoded['active_count'] ?? 0;
          _inactivePlayers = decoded['inactive_count'] ?? 0;
        });
      } else {
        _snack('Failed to load events', error: true);
      }
    } catch (e) {
      print("FETCH ERROR: $e");
  ...
  Future<void> uploadPlayers(List<PlayerModel> players) async {
    final token = html.window.localStorage["token"];

    final response = await http.post(
      Uri.parse("https://dev.sports-next.com/api/players/bulk-upload"),
      headers: {
        "Authorization": "Bearer $token",
        "Content-Type": "application/json",
      },
      body: jsonEncode({
        "players": players.map((p) {
          return {"full_name": "${p.firstName} ${p.lastName}"};
        }).toList(),
      }),
    );
    ...
    if (response.statusCode == 200) {
      ...
      _snack("Players imported successfully");
    } else {
      print("STATUS: ${response.statusCode}");
      print(utf8.decode(response.bodyBytes));

      _snack("Bulk Upload failed", error: true);
    }
  }

  Future<void> fetchDuplicatesPlayers() async {
    try {
      final response = await http.get(
        Uri.parse("https://dev.sports-next.com/api/players/duplicates"),
        headers: {
          "Authorization": "Bearer $token",
          "Content-Type": "application/json",
        },
      ...
      if (response.statusCode == 200) {
        ...
        List<List<PlayerModel>> groups = [];

        for (var group in decoded) {
          List<PlayerModel> players = [];

          for (var p in group) {
            players.add(PlayerModel.fromJson(p));
          }
        ...
        print("Failed to load dulpicates");
      }
    } catch (e) {
      print("DUPLICATE FETCH ERROR: $e");
    }
  }

  Future<void> mergePlayers(List<PlayerModel> group) async {
    final token = html.window.localStorage["token"];

    print("MERGE PAYLOAD: ${group.map((p) => p.id).toList()}");

    final response = await http.post(
      Uri.parse("https://dev.sports-next.com/api/players/merge-duplicates"),
      headers: {
        "Authorization": "Bearer $token",
        "Content-Type": "application/json",
      },
      body: jsonEncode({"player_ids": group.map((p) => p.id).toList()}),
    );

    if (response.statusCode == 200) {
      await fetchPlayers();
      await fetchDuplicatesPlayers();

      _snack("Duplicate players merged successfully");
    } else {
      print(response.body);

      _snack("Merge failed", error: true);
    }
  }
  ...
  Future<void> togglePlayerStatus(String playerId) async {
    ...
    final response = await http.post(
      Uri.parse("$baseUrl/players/$playerId/toggle-active"),
      headers: {
        "Authorization": "Bearer $token",
        "Content-Type": "application/json",
      },
    ...
    if (response.statusCode == 200) {
      await fetchPlayers(page: _currentPage);
    }
  }
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        'Tip: Use Deactivate to keep history intact.',
                        style: TextStyle(
                          fontSize: 11,
                ...
                children: [
                  ...
                  Expanded(
                    child: ElevatedButton(
                      onPressed: () async {
                        await http.delete(
                          Uri.parse("$baseUrl/players/${p.id}"),
                          headers: {
                            "Authorization": "Bearer $token",
                            "Content-Type": "application/json",
                          },
  ...
  void _showMerge(List<PlayerModel> group) {
    showDialog(
      ...
      builder: (dialogContext) => _MergeDialog(
        group: group,
        onMerge: (PlayerModel primary, List<PlayerModel> others) async {
          await mergePlayers([primary, ...others]);
  ...
  void _showHistory(PlayerModel p) {
    showDialog(
      context: context,
      builder: (_) => _HistoryDialog(player: p),
    );
  }
  ...
  void _showBulkUpload() {
    showDialog(
      ...
      builder: (_) => _BulkUploadDialog(
        onImport: (List<PlayerModel> imported) async {
          await uploadPlayers(imported);
        },
      ),
    );
  }
  ...
  void _openAdd() async {
    ...
    if (result == true) {
      await fetchPlayers(); // 🔥 refresh instantly
    }
  }
  ...
  void _openEdit(PlayerModel p) {
    showDialog(
      ...
      builder: (_) => EditPlayerScreen(
        player: p,
        onSave: (PlayerModel updated) async {
          await fetchPlayers();
            ...
            children: [
              Column(
                ...
                children: [
                  ...
                  Text(
                    "Manage player profiles, history and deduplication",
                  ...
                  child: Column(
                    children: [
                      ...
                      TabBar(
                        ...
                        indicatorWeight: 2.5,
                        tabs: [
                          Tab(text: "All Players  ($_totalPlayers)"),
                    ...
                    children: [
                      ...
                      _filtered.isEmpty
                          ...
                          : _PlayersTable(
                              ...
                              onToggle: (p) => togglePlayerStatus(p.id),
                      ...
                      dupGroups.isEmpty
                          ? const _EmptyState(
                              msg: "No duplicate players detected 🎉",
        ...
        Expanded(
          child: ListView.builder(
            ...
            itemBuilder: (_, i) => _PlayerRow(
              ...
              onToggle: (id) => onToggle(players[i]),
  ...
  Widget build(BuildContext context) {
    ...
    return MouseRegion(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: _hov
              ? const Color(0xFF0A46D8).withOpacity(0.03)
              : Colors.white,
                ...
                children: [
                  ...
                  _ActionBtn(
                    ...
                    color: p.isActive
                        ? const Color(0xFF6B7280)
                        : const Color(0xFF16A34A),
                    onTap: () => widget.onToggle(widget.player.id),
                  ),
                  const SizedBox(width: 4),
                  _ActionBtn(
                ...
                child: Row(
                  children: [
                    ...
                    Column(
                      ...
                      children: [
                        Text(
                          "${group.length} Duplicate Players",
                          style: const TextStyle(
                            fontSize: 13,
                            fontWeight: FontWeight.bold,
                        ...
                        Text(
                          "Potential duplicate players detected",
                          style: TextStyle(
                            fontSize: 11,
                            color: Colors.amber.shade700,
                          ),
...
class _MergePlayersTabState extends State<_MergePlayersTab> {
  ...
  void _removeSlot(bool isA) => setState(() {
    if (isA)
      _slotA = null;
    else
      _slotB = null;
  ...
  Widget _idleHint() => Center(
    child: Column(
      ...
      children: [
        ...
        Text(
          "Type a name, email or phone to find players",
...
class _MergeDialog extends StatefulWidget {
  ...
  final Future<void> Function(PlayerModel primary, List<PlayerModel> others)
  onMerge;
...
class _MergeDialogState extends State<_MergeDialog> {
  ...
  void initState() {
    ...
    for (int i = 1; i < widget.group.length; i++) {
      if (widget.group[i].totalMatches > widget.group[best].totalMatches)
        best = i;
    }
    _primaryIdx = best;
  }
                  ...
                  const Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          "Select the primary record to keep. Match history will be combined.",
                          style: TextStyle(
                            fontSize: 12,
                            color: Color(0xFF6B7280),
                          ),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child: Text(
                        "The primary record's profile data is preserved. "
                            ...
                            children: [
                              ...
                              Column(
                                ...
                                children: [
                                  ...
                                  Text(
                                    "Added ${p.createdAt.length > 10 ? p.createdAt.substring(0, 10) : p.createdAt}",
                                    style: TextStyle(
                                      fontSize: 10,
                                      color: Colors.grey.shade400,
                                    ),
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _MergePoint(
                            ...
                            color: const Color(0xFF16A34A),
                            text:
                                'Primary record "${_primary.fullName}" is kept with its profile data',
                          ),
                          _MergePoint(
                            icon: Icons.merge_outlined,
                            color: const Color(0xFF0A46D8),
                            text:
                                'All ${widget.group.fold(0, (s, p) => s + p.totalMatches)} match history records are combined under the primary',
                          ),
                          _MergePoint(
                            icon: Icons.delete_outline,
                            color: const Color(0xFFEF4444),
                            text:
                                '${_others.length} duplicate record${_others.length > 1 ? "s" : ""} will be permanently removed',
                          ),
                        ],
                      ),
                    ),
                ...
                children: [
                  ...
                  Expanded(
                    child: ElevatedButton.icon(
                      ...
                      label: Text(
                        "Merge ${widget.group.length} Records",
                        style: const TextStyle(
                          fontWeight: FontWeight.w600,
                          fontSize: 14,
                        ),
                      ...
                      onPressed: isLoading
                          ...
                          : () async {
                              ...
                              await widget.onMerge(_primary, _others);
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            _Mini(
                              label: "${player.totalMatches} Matches",
                              color: const Color(0xFF0A46D8),
                            ),
                            const SizedBox(width: 6),
                            _Mini(
...
class _BulkUploadDialogState extends State<_BulkUploadDialog> {
  ...
  void _parseContent(String content, String fname) {
    ...
    for (int i = startRow; i < lines.length; i++) {
      ...
      if (p == null) {
        errors.add(
          'Row ${i + 1}: skipped — "${lines[i]}" (not enough columns)',
        );
  ...
  Future<void> _pickFile() async {
    try {
      final result = await FilePicker.platform.pickFiles(
        ...
        withData:
            true, // reads bytes immediately (works on web + mobile + desktop)
      );
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                        ...
                        child: Container(
                          ...
                          child: Column(
                            children: [
                              ...
                              const SizedBox(height: 4),
                              Text(
                                "Opens your file explorer — select a .csv file",
                                style: TextStyle(
                                  fontSize: 12,
                                  color: Colors.grey.shade500,
                                ),
                          ...
                          children: [
                            Row(
                              children: [
                                ...
                                const SizedBox(width: 6),
                                Text(
                                  "${_errors.length} row${_errors.length > 1 ? 's' : ''} skipped",
                                  style: TextStyle(
                                    fontSize: 12,
                                    fontWeight: FontWeight.bold,
                    ...
                    // Preview table
                    if (_step == _UploadStep.previewing &&
                        _preview.isNotEmpty) ...[
                      const SizedBox(height: 20),
                      Divider(color: Colors.grey.shade100),
                      const SizedBox(height: 16),
                      ...
                      Container(
                        ...
                        child: Row(
                          children: [
                            ...
                            const SizedBox(width: 10),
                            Text(
                              "${_preview.length} players ready to import",
                              style: const TextStyle(
                                fontSize: 13,
                                fontWeight: FontWeight.w600,
                          ...
                          children: [
                            ...
                            if (_preview.length > 5)
                              Padding(
                                ...
                                child: Text(
                                  "...and ${_preview.length - 5} more players",
                                  style: TextStyle(
                                    fontSize: 12,
                                    color: Colors.grey.shade400,
                                  ),
                ...
                children: [
                  ...
                  Expanded(
                    child: ElevatedButton.icon(
                      ...
                      onPressed:
                          _step == _UploadStep.previewing && _preview.isNotEmpty
                          ? () {
        ...
        Expanded(
          child: RichText(
            text: TextSpan(
              ...
              children: [
                TextSpan(
                  text: "$count duplicate group${count > 1 ? 's' : ''} found. ",
                  style: const TextStyle(fontWeight: FontWeight.bold),
                ),
                const TextSpan(
                  text: "Review and merge them to keep your player data clean.",
                ),
              ],
            ),
          ),
